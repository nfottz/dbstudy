/*
    ...
    2. 형식
        ...
        BEGIN
            수행할 PL/SQL
        END;
    3. 변수에 저장된 값을 출력하기 위해서 서버 출력을 ON 시켜주어야 한다.
...
-- 2. 참조 변수
-- 특정 칼럼의 타입을 그대로 사용
-- SELECT절의 INTO를 이용해 테이블의 데이터를 가져와서 변수에 저장할 수 있다.
...
BEGIN
    ...
    SELECT SALARY
     ...
     IF SAL >= 15000 THEN MESSAGE := '고액연봉';
     ELSE MESSAGE := '보통연봉';
     END IF;
...
BEGIN
    ...
    SELECT NVL(COMMISSION_PCT, 0), SALARY
     ...
     IF COMM_PCT = 0 THEN MESSAGE := '커미션없음';
     ELSE MESSAGE := TO_CHAR(COMM_PCT * SAL);
     END IF;
     DBMS_OUTPUT.PUT_LINE('사원번호 '||EMP_ID||'인 사원의 커미션 : '||MESSAGE);
END;
...
BEGIN
    ...
    CASE
        WHEN PHONE = '011' THEN MESSAGE := 'MOBILE';
        WHEN PHONE = '515' THEN MESSAGE := 'EAST';
        WHEN PHONE = '590' THEN MESSAGE := 'WEST';
        WHEN PHONE = '603' THEN MESSAGE := 'SOUTH';
        WHEN PHONE = '650' THEN MESSAGE := 'NORTH';
    END CASE;
...
-- 1 ~ 10 사이 정수를 '짝수', '홀수', '3의 배수'로 출력하시오.
DECLARE
    N NUMBER(2);
...
BEGIN
    FOR N IN 1..10 LOOP
        SELECT MOD(N, 3)
          INTO MODULAR
          FROM DUAL;
        IF MODULAR = 0 THEN MESSAGE := '3의배수';
        ELSE
            SELECT MOD(N, 2)
              INTO MODULAR
              FROM DUAL;
            IF MODULAR = 1 THEN MESSAGE := '홀수';
            ELSE MESSAGE := '짝수';
            END IF;
        END IF;
        DBMS_OUTPUT.PUT_LINE(N||'은(는) '||MESSAGE);
    END LOOP;
END;
...
BEGIN
    ...
    FOR EMP_ID IN 100..206 LOOP
        ...
        CNT := CNT + 1;
    END LOOP;
    DBMS_OUTPUT.PUT_LINE(ROUND(TOTAL / CNT, 2));
END; 

-- DEPARTMENT_ID가 50인 사원들의 목록을 DEPT50 테이블로 복사하시오.
-- 1) DEPT50 테이블 만들기
-- 2) 행 변수로 EMPLOYEES 테이블의 정보 읽기
-- 3) DEPARTMENT_ID가 50이면 행 변수에 저장된 내용을 DEPT50 테이블로 INSERT 하기

DROP TABLE DEPT50;
CREATE TABLE DEPT50
    AS (SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME, EMAIL, PHONE_NUMBER, HIRE_DATE, JOB_ID, SALARY
           , COMMISSION_PCT, MANAGER_ID, DEPARTMENT_ID
         FROM EMPLOYEES
        WHERE 1 = 2);
...
DECLARE
    EMP EMPLOYEES%ROWTYPE;
BEGIN
    FOR EMP IN (SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME, EMAIL, PHONE_NUMBER, HIRE_DATE, JOB_ID, SALARY
                     , COMMISSION_PCT, MANAGER_ID, DEPARTMENT_ID
                  FROM EMPLOYEES
...
-- 1부터 정수값을 누적하시오. 누적 값이 100을 초과하면 그만 누적하고 
DECLARE
    N NUMBER;
    TOTAL NUMBER;
BEGIN
    ...
    TOTAL := 0;
    WHILE TRUE LOOP
        IF TOTAL > 100 THEN EXIT;
        END IF;
        N := N + 1;
        TOTAL := TOTAL + N;

    END LOOP;
    DBMS_OUTPUT.PUT_LINE('1부터 '||N||'까지의 합 : '||TOTAL);
END;
