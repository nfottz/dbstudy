...
SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME, DEPARTMENT_ID
  FROM EMPLOYEES
 WHERE DEPARTMENT_ID IS NULL;   -- ISNOT NULL
...
-- 8. EMPLOYEES 테이블을 FIRST_NAME의 가나다 순(오름차순:ASCENDING SORT)으로 정렬해서 조회하기
SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME
  FROM EMPLOYEES
 ORDER BY FIRST_NAME ASC;   -- ASC는 생략 가능하다.
 
-- 9. EMPLOYEES 테이블을 높은 SALARY를 받는 사원 순(내림차순:DESCENDING SORT)으로 조회하기
SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME, SALARY
  FROM EMPLOYEES
 ORDER BY SALARY DESC;  -- DESC는 생략이 불가능하다.
 
-- 10. EMPLOYEES 테이블의 사원들을 DEPARTMENT_ID 순으로 조회하고, 동일한 부서 내에서 높은 SALARY 순으로 조회하기
