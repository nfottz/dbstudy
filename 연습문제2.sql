...
-- 장미란  62000
SELECT A.고객명, A.총구매액
  FROM (SELECT ROW_NUMBER() OVER(ORDER BY SUM(B.PRICE * O.AMOUNT) DESC) AS ROWNO
             , C.CUSTOMER_NAME AS 고객명, SUM(O.AMOUNT * B.PRICE) AS 총구매액
          FROM CUSTOMER_TBL C INNER JOIN ORDER_TBL O
            ON C.CUSTOMER_ID = O.CUSTOMER_ID INNER JOIN BOOK_TBL B
            ON O.BOOK_ID = B.BOOK_ID
         GROUP BY C.CUSTOMER_NAME, C.CUSTOMER_ID) A
 WHERE ROWNO BETWEEN 2 AND 3;

/*
SELECT C.CUSTOMER_NAME, SUM(O.AMOUNT * B.PRICE)
  FROM CUSTOMER_TBL C INNER JOIN ORDER_TBL O
    ON C.CUSTOMER_ID = O.CUSTOMER_ID INNER JOIN BOOK_TBL B
    ON O.BOOK_ID = B.BOOK_ID
 GROUP BY C.CUSTOMER_NAME, C.CUSTOMER_ID
 ORDER BY SUM(O.AMOUNT * B.PRICE) DESC;
 */