...
-- 추신수  86000
-- 장미란  62000
SELECT C.CUSTOMER_NAME AS 고객명, SUM(O.AMOUNT * B.PRICE) AS 총구매액
  FROM CUSTOMER_TBL C INNER JOIN ORDER_TBL O
    ON C.CUSTOMER_ID = O.CUSTOMER_ID INNER JOIN BOOK_TBL B
    ON O.BOOK_ID = B.BOOK_ID
 GROUP BY C.CUSTOMER_NAME, C.CUSTOMER_ID;

SELECT ROWNUM AS RN, C.CUSTOMER_NAME, O.AMOUNT, B.PRICE
  FROM CUSTOMER_TBL C INNER JOIN ORDER_TBL O
    ON C.CUSTOMER_ID = O.CUSTOMER_ID INNER JOIN BOOK_TBL B
    ON O.BOOK_ID = B.BOOK_ID
 GROUP BY C.CUSTOMER_NAME, C.CUSTOMER_ID
...
SELECT C.CUSTOMER_NAME AS 고객명, SUM(O.AMOUNT * B.PRICE) AS 총구매액
  FROM (SELECT ROWNUM AS RN, C.CUSTOMER_NAME, O.AMOUNT, B.PRICE
          FROM CUSTOMER_TBL C INNER JOIN ORDER_TBL O
            ON C.CUSTOMER_ID = O.CUSTOMER_ID INNER JOIN BOOK_TBL B
            ON O.BOOK_ID = B.BOOK_ID
         GROUP BY C.CUSTOMER_NAME, C.CUSTOMER_ID
