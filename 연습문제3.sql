...
UPDATE PRODUCTS SET PROD_CATEGORY = '서적' WHERE PROD_NAME = '책';
COMMIT;
...
-- 박수홍  메모리  800
-- 박수홍  모니터  1000
SELECT U.USER_NAME AS 고객명, P.PROD_NAME AS 제품명, SUM(P.PROD_PRICE * B.BUY_AMOUNT) AS 구매액
  FROM USERS U INNER JOIN BUYS B
    ON U.USER_ID = B.USER_ID INNER JOIN PRODUCTS P
    ON B.PROD_CODE = P.PROD_CODE
 WHERE P.PROD_CATEGORY = '전자'
 GROUP BY U.USER_NAME, P.PROD_NAME
 ORDER BY U.USER_NAME;
...
-- 강호동  3
SELECT 고객명, 구매횟수
  FROM (SELECT U.USER_NAME AS 고객명, COUNT(B.BUY_NO) AS 구매횟수
          FROM USERS U INNER JOIN BUYS B
            ON U.USER_ID = B.USER_ID
...
-- 청바지  2
-- 노트북  1
SELECT P.PROD_NAME AS 제품명, NVL(COUNT(B.BUY_NO), 0) AS 판매횟수
