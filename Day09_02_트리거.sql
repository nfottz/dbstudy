...
CREATE OR REPLACE TRIGGER TRIG2
    AFTER
    INSERT OR UPDATE OR DELETE  -- 순서 무관
    ON EMPLOYEES
    FOR EACH ROW
BEGIN
...
CREATE OR REPLACE TRIGGER TRIG3
    ...
    FOR EACH ROW
BEGIN
    IF UPDATING THEN DBMS_OUTPUT.PUT_LINE('수정 전 LAST_NAME :'||:OLD.LAST_NAME);
    ELSIF DELETING THEN DBMS_OUTPUT.PUT_LINE('삭제 전 LAST_NAME :'||:OLD.LAST_NAME);
    END IF;
END;
...
-- 2) OLD 테이블의 데이터를 모두 퇴사자 테이블로 삽입한다.
-- 3) 퇴사번호는 시퀀스, 퇴사일자는 SYSDATE로 처리해서 삽입한다.
    INSERT INTO RETIRES(RETIRE_ID, RETIRE_DATE
                      , EMPLOYEE_ID, FIRST_NAME, LAST_NAME, EMAIL, PHONE_NUMBER
                      , HIRE_DATE, JOB_ID, SALARY, COMMISSION_PCT, MANAGER_ID, DEPARTMENT_ID)
        VALUES(RETIRE_SEQ.NEXTVAL, SYSDATE
