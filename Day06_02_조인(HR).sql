...
-- 동일한 부서에서 근무하는 사원들을 조인하기 위해서 DEPARTMENT_ID로 조인 조건 생성

-- 사원(나)         사원(다른사람)
-- EMPLOYEES ME     EMPLOYESS YOU

-- 같은 부서에 근무하는 사원 중에서 나보다 SALARY가 높은 사원 정보를 조회하시오.
...
-- 1. LOCATION_ID가 1700인 부서에 근무하는 사원들의 EMPLOYEE_ID, FIRST_NAME, DEPARTMENT_NAME 조회
-- 1) 표준 문법
...
SELECT D.DEPARTMENT_NAME, COUNT(*) AS 사원수, ROUND(AVG(E.SALARY)) AS 평균연봉
...
-- 6. 모든 부서의 DEPARTMENT_NAME과 근무 중인 사원 수를 조회하시오.
--    근무하는 사원이 없으면 0으로 조회하시오.
