...
SELECT EMPLOYEE_ID
     ...
     , HIRE_DATE
  FROM EMPLOYEES
 WHERE TO_CHAR(TO_DATE(HIRE_DATE, 'YY/MM/DD'), 'YY') = '05';
...
SELECT EMPLOYEE_ID
     , FIRST_NAME || ' ' || LAST_NAME AS 이름
     , CEIL(MONTHS_BETWEEN(SYSDATE, TO_DATE(HIRE_DATE, 'YY/MM/DD'))) AS 근무개월수
  FROM EMPLOYEES;
...
SELECT DEPARTMENT_ID AS 부서번호, JOB_ID AS 직무, COUNT(*) AS 사원수
  FROM EMPLOYEES
...
SELECT SUBSTR(DEPARTMENT_NAME, 1, 2) AS 첫두글자
     , COUNT(*) OVER(PARTITION BY SUBSTR(DEPARTMENT_NAME, 1, 2)) AS 그룹별사원수
  FROM DEPARTMENTS;
