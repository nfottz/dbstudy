...
INSERT INTO PROD_TBL(P_CODE, P_NAME, P_PRICE, P_STOCK)
    VALUES(1000, '홈런볼', 1000, 100);
INSERT INTO PROD_TBL(P_CODE, P_NAME, P_PRICE, P_STOCK)
    VALUES(1001, '맛동산', 2000, 100);
COMMIT;
...
INSERT INTO CUST_TBL(C_NO, C_NAME, C_POINT)
    VALUES(1, '정숙', 0);
INSERT INTO CUST_TBL(C_NO, C_NAME, C_POINT)
    VALUES(2, '재홍', 0);
COMMIT;
...
BEGIN
    -- 1) 구매내역 테이블에 구매 내역을 추가(INSERT)한다.
    INSERT INTO BUY_TBL(B_NO, C_NO, P_CODE, B_AMOUNT)
        VALUES(BUY_SEQ.NEXTVAL, CNO, PCODE, BUY_AMOUNT);
    
    -- 2) 제품 테이블의 재고 내역을 수정(UPDATE)한다.
    UPDATE PROD_TBL SET P_STOCK = (P_STOCK - BUY_AMOUNT) WHERE P_CODE = PCODE;
    
    -- 3) 고객 테이블의 포인트를 수정(UPDATE)한다.
    --    총 구매액의 10%를 정수로 올림처리하여 포인트로 준다.
    UPDATE CUST_TBL
        SET C_POINT = CEIL((SELECT P_PRICE FROM PROD_TBL WHERE P_CODE = PCODE) * BUY_AMOUNT * 0.1)
        WHERE C_NO = CNO;
...
-- BUY_PROC 프로시저 호출
EXECUTE BUY_PROC(1, 1000, 2);   -- 고객번호 1, 제품코드 1000, 구매수량 10
BEGIN
